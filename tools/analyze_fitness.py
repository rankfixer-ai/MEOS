...
# Get top 50 alleles with their environment scores
rows = conn.execute("""
    SELECT fitness_score, environment_small, environment_medium, environment_large
    FROM alleles 
    WHERE gene_id = ? AND fitness_score IS NOT NULL AND environment_small IS NOT NULL
    ORDER BY fitness_score DESC
    LIMIT 50
""", (gene["id"],)).fetchall()

# Decompose: fitness is avg of three environments
# Each environment score = weighted combination of latency, accuracy, cost, reliability
# We can see how much variance exists in each environment
